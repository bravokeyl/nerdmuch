...
add_action( 'widgets_init', 'blog_category_two_widget' );

function blog_category_two_widget() {register_widget( 'blog_category_two_sci1' );}

class blog_category_two_sci1 extends WP_Widget {

	/* Register widget with WordPress. */

	function __construct() {
		parent::__construct(
			'blog_category_two_sci1', // Widget ID
			__('Blogroll 2', 'science-magazine'), // Name
		...
		/* Default widget settings. */
			...
			$defaults = array( 'title' => 'blogroll2', 'number' => 4, 'offset' => 0, 'author' => '0', 'date' => '0', 'widget_size' => 'two-parts', 'categories' => '0', 'excerptnumber'=>'30', 'cat_show' => 'on', 'navigation'=>'0', 'auto_load' => '0');
...
<div class="blog-category blogroll2">
	<ul>
		<?php
		...
		<li <?php post_class((is_sticky()?'sticky':'')); ?>>
			...
			<div class="blogwrap">
				...
				<div class="category-icon">
					<?php $category = get_the_category(); if($category[0]){echo '<a href="'.esc_url(get_category_link($category[0]->term_id )).'" title="'.esc_attr($category[0]->cat_name).'" class="bk-cat-item-'.esc_attr($category[0]->slug).'">'.esc_html($category[0]->cat_name).'</a>';} ?>
				</div>
				<!--featured-category-->
				<?php } ?>
